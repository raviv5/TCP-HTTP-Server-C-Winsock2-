...
void run()
{
	...
	while (true)
	{
		...
		//check for socekets that pass the 2 minutes mark

		for (int i = 0; i < MAX_SOCKETS; i++)
		{
			time_t timeNow = time(NULL);
			if (difftime(timeNow, sockets[i].activeLastIn) > 120 && sockets[i].recv == RECEIVE)
			{
				cout << "Server: Connection timed out pass 2 minutes mark since last request. Closing Connection. " << sockets[i].id << endl;
...
bool addSocket(SOCKET id, int what)
{
	for (int i = 0; i < MAX_SOCKETS; i++)
	{
		...
		{
			sockets[i].activeLastIn = time(NULL);
			sockets[i].id = id;
			sockets[i].recv = what;
			sockets[i].send = IDLE;
			sockets[i].len = 0;
...
void receiveMessage(int index)
{
	...
	else
	{
		...
		sockets[index].activeLastIn = time(NULL);
...
void sendMessage(int index)
{
	...
	Request request = Request::Parse(sockets[index].buffer, sockets[index].len);
	//get the string of the response
	sendBuff = Response::To(request).ToString();
	...
	//reset the buffer for new request.
	sockets[index].len = 0;
